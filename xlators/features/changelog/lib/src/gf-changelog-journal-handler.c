...
/**
 * This is dirty and painful as of now till there is event filtering in the
 ...
 * (selected at the time of probe). As of now this is complete BS.
 *
 * FIXME: cleanup this bugger once server filters events.
 */
int
gf_changelog_handle_journal (void *xl, char *brick,
                             void *cbkdata, struct iovec **vector, int count)
{
        int                       i        = 0;
        int                       ret      = 0;
        int                       evsize   = 0;
        ...
        for (i = 0; i < count; i++) {
                event = (changelog_event_t *)vector[i]->iov_base;
                evsize = vector[i]->iov_len / CHANGELOG_EV_SIZE;

                for (; evsize > 0; evsize--, event++) {
                        if (event->ev_type == CHANGELOG_OP_TYPE_JOURNAL) {
                                ret = gf_changelog_queue_journal (jnl_proc,
                                                                  event);
                                if (ret)
                                        gf_log (this->name, GF_LOG_ERROR,
...
static int
gf_changelog_open_dirs (gf_changelog_journal_t *jnl)
{
        int  ret                    = -1;
        DIR *dir                    = NULL;
        int  tracker_fd             = 0;
                           ...
                           char *brick_path, char *scratch_dir)
{
        ...
        ret = gf_changelog_open_dirs (jnl->hist_jnl);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR,
...
gf_changelog_journal_init (void *xl, struct gf_brick_spec *brick)
{
        ...
        ret = gf_changelog_open_dirs (jnl);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR,
