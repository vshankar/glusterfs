...
#define ROT_BUFF_COUNT  2
#define ROT_BUFF_ALLOC_SIZE  4200 /* change this later to 128k */
...
typedef struct rbuf_list {
        ...
        unsigned long seq[2];            /* if interested, this should store
...
#define rvec_for_each_entry(pos, rlist)                                 \
        for (pos = list_entry (rlist->veclist.next, typeof(*pos), list); \
             rlist->used > 0;                                           \
