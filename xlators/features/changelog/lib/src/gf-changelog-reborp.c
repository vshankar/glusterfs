                                   ...
                                   rpcsvc_event_t event, void *data)
{
        ...
        case RPCSVC_EVENT_ACCEPT:
                ...
                if (entry->connected)
                        entry->connected (this, entry->brick, entry->ptr);
                break;
        case RPCSVC_EVENT_DISCONNECT:
                if (entry->disconnected)
                        entry->disconnected (this, entry->brick, entry->ptr);
                break;
        default:
                break;
        }
...
{
        return entry->callback (entry->this, entry->brick,
                                entry->ptr, vec, payloadcnt);
}
...
gf_changelog_callback_invoker (void *arg)
{
        ...
        while (1) {
                ...
                ret = gf_changelog_invoke_callback (entry, &vec, event->count);
                if (ret != 0)
                        gf_log (this->name, GF_LOG_WARNING,
                                "callback invocation returned: %d", ret);
                                    ...
                                    xlator_t *this, gf_changelog_t *entry)
{
        ...
        gf_log (this->name, GF_LOG_DEBUG,
                "seq: %lu (time: %lu.%lu), (vec: %d, len: %ld)",
                rpc_req.seq, rpc_req.tv_sec,
                                      ...
                                      xlator_t *this, gf_changelog_t *entry)
{
        ...
        ret = gf_changelog_invoke_callback (entry, &vec, payloadcnt);
        if (ret != 0)
                goto handle_xdr_error;
