...
int
gf_probe_changelog_simple (call_frame_t *frame, xlator_t *this, void *data)
{
        int ret = 0;
        char *sock = NULL;
...
gf_probe_changelog_filter (call_frame_t *frame, xlator_t *this, void *data)
{
        ...
        (void) memcpy (&req.sock, sock, strlen (sock));
        req.filter = (CHANGELOG_OP_TYPE_OPEN | CHANGELOG_OP_TYPE_RELEASE);
...
struct rpc_clnt_procedure gf_changelog_procs[CHANGELOG_RPC_PROC_MAX] = {
        ...
        [CHANGELOG_RPC_PROBE_SIMPLE] = {
                "PROBE SIMPLE", gf_probe_changelog_simple
        },
