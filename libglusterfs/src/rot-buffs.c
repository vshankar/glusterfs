...
inline void
rbuf_free_rvec (rbuf_iovec_t *rvec)
{
        if (!rvec)
                return;
        list_del (&rvec->list);
...
inline void
rlist_purge_all_vec (rbuf_list_t *rlist)
{
        rbuf_iovec_t *rvec = NULL;
...
rbuf_init (int bufcount)
{
 ...
 dealloc_rbuf_list:
        ...
        pthread_spin_destroy (&rbuf->lock);
 dealloc_rbuf:
        GF_FREE (rbuf);
 error_return:
        return NULL;
...
rbuf_alloc_write_area (rbuf_list_t *rlist, size_t bytes)
{
        ...
        if (list_last_entry (&rlist->veclist,
                             rbuf_iovec_t, list) == rlist->rvec) {
