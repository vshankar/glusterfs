...
int
__glusterd_bitd_rpc_notify (struct rpc_clnt *rpc, void *mydata,
                             rpc_clnt_event_t event, void *data)
{
        xlator_t                *this           = NULL;
        glusterd_conf_t         *conf           = NULL;
        ...
        case RPC_CLNT_CONNECT:
                ...
                (void) glusterd_bitd_set_online_status (volinfo, _gf_true);

                break;

        case RPC_CLNT_DISCONNECT:
                if (glusterd_is_bitd_online (volinfo)) {
                        gf_msg (this->name, GF_LOG_INFO, 0,
                                GD_MSG_NODE_DISCONNECTED,
                                "snapd for volume %s has disconnected from "
                                "glusterd.", volinfo->volname);

                        (void) glusterd_bitd_set_online_status
                                                          (volinfo, _gf_false);
                }
                break;
...
int
glusterd_bitd_rpc_notify (struct rpc_clnt *rpc, void *mydata,
                             rpc_clnt_event_t event, void *data)
{
        return glusterd_big_locked_notify (rpc, mydata, event, data,
                                           __glusterd_bitd_rpc_notify);
}
